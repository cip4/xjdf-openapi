...
public class JsonSchemaConverter
{
	...
	{
		final SchemaReader reader = new SchemaReader();
	...
	public Schemas convert(OutputStream xjdfOutputStream, OutputStream xjmfOutputStream)
	{
		...
		final Schemas schemaMap = new Schemas();
		final Map<String, String> xjdfTypes = new HashMap<>();

		if (xjdfOutputStream != null)
		{
			final Schema xjdfSchema = convertModel("XJDF");
		...
		if (xjmfOutputStream != null)
		{
			final Schema xjmfSchema = convertModel("XJMF", xjdfTypes);
	...
	{
		final TypeTranslator nameTranslator = new TypeTranslator(PREFIX, referencedTypes);
		final Schemas defs = new Schemas();
		convertNamedElement(root, nameTranslator, defs);
		final Schema xjdfSchema = defs.remove(root);
	...
	private void convertNamedType(String typeName, TypeTranslator nameTranslator, Schemas schemas)
	{
		...
		try
		{
			final Node typeNode = (Node) xPath.evaluate(
					"/xs:schema/xs:simpleType[@name='" + typeName + "'] | /xs:schema/xs:complexType[@name='" + typeName + "']", doc, XPathConstants.NODE);

			if ("simpleType".equals(typeNode.getLocalName()))
			{
	...
	{
		final Model schema = SimpleType.create(typeNode, new Context(xPath, nameTranslator, typeNode)).getModel();
	...
	{
		final Model schema = new ComplexType(new Context(xPath, nameTranslator, typeNode)).getModel();
		schemas.put(schema.getName(), schema.getSchema());
	...
	private void convertNamedElement(String elementName, TypeTranslator nameTranslator, Schemas schemas)
	{
		try
		{
			final Node element = (Node) xPath.evaluate("/xs:schema/xs:element[@name='" + elementName + "']", doc, XPathConstants.NODE);

			convertReferencedTypes(element, nameTranslator, schemas);
			convertSubstituteElements(elementName, nameTranslator, schemas);

			if ((element.getAttributes().getNamedItem("abstract") != null && "true".equals(element.getAttributes().getNamedItem("abstract").getNodeValue()))
					|| element.getAttributes().getNamedItem("substitutionGroup") != null)
			{
				return;
			}

			final Model schema = new NamedElement(new Context(xPath, nameTranslator, element)).getModel();
			schemas.put(schema.getName(), schema.getSchema());
	...
	private void convertReferencedTypes(Node node, TypeTranslator nameTranslator, Schemas schemas)
	{
		try
		{
			final NodeList types = (NodeList) xPath.evaluate(".//@type | .//@itemType", node, XPathConstants.NODESET);

			for (int i = 0; i < types.getLength(); i++)
			{
				final String typeName = types.item(i).getNodeValue();
	...
	private void convertExtendedTypes(Node element, TypeTranslator nameTranslator, Schemas schemas)
	{
		try
		{
			final NodeList elements = (NodeList) xPath.evaluate(".//@base", element, XPathConstants.NODESET);
	...
	private void convertReferencedElements(Node node, TypeTranslator nameTranslator, Schemas schemas)
	{
		try
		{
			final NodeList elements = (NodeList) xPath.evaluate(".//xs:element/@ref", node, XPathConstants.NODESET);

			for (int i = 0; i < elements.getLength(); i++)
			{
				final String elementName = elements.item(i).getNodeValue();
	...
	private void convertSubstituteElements(String elementName, TypeTranslator nameTranslator, Schemas schemas)
	{
		try
		{
			final NodeList elements = (NodeList) xPath.evaluate("/xs:schema/xs:element[@substitutionGroup='" + elementName + "']/@name", doc,
