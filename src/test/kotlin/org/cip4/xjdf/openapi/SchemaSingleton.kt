...
object SchemaSingleton {
    ...
    private fun getSchemaForPath(path: String): JsonNode {
        return openApiSpec.schemaNode["paths"][path]
    }

    fun getSchemaForRequest(path: String): JsonSchema {
        val schema = getSchemaForPath(path)["post"]["requestBody"]["content"].first()["schema"]
        return factory.getSchema(schema, config)
    }

    fun getSchemaForResponse(path: String, code: Int = 200): JsonSchema {
        val schema = getSchemaForPath(path)["post"]["responses"][code.toString()]["content"].first()["schema"]
        return factory.getSchema(schema, config)
    }

    fun getSchemaForSignal(path: String, code: Int = 200): JsonSchema {
